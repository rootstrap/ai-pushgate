...
test("does not expose a separate push command", async () => {
  const result = await runRunner(["push"]);

  assert.equal(result.code, 64, formatResult(result));
  assert.match(result.stderr, /Unsupported Pushgate command: push/);
  assert.doesNotMatch(result.stderr, new RegExp("pushgate " + "push"));
});
