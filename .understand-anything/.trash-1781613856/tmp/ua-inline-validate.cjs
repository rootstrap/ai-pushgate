...
try {
  ...
  graph.layers.forEach(layer => {
    if (!layer.id || !layer.name || !layer.description || !Array.isArray(layer.nodeIds)) issues.push('Layer missing required fields: ' + JSON.stringify(layer));
  ...
  graph.tour.forEach((step, i) => {
    if (typeof step.order !== 'number' || !step.title || !step.description || !Array.isArray(step.nodeIds)) issues.push('Tour step[' + i + '] missing required fields');
